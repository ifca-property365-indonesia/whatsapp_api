...
@Injectable()
export class ReminderThreeService {
  private readonly logger = new Logger(ReminderThreeService.name);
  ...
  async dataReminder(ReminderThreeDto: ReminderThreeDto) {
    /* ===============================
    * 1. SET LINK
    * =============================== */
    const link =
      ReminderThreeDto.environment === 'sandbox'
    ...
    * =============================== */
    const payload = {
      ...
      recipient: ReminderThreeDto.wa_no,
      type: 'template',
      template: {
            ...
            parameters: [
              {
                type: 'document',
                document: {
                  link: `${link}${ReminderThreeDto.file_name}`,
                  filename: ReminderThreeDto.file_name,
                },
              },
            ],
          },
    ...
    retry(
      () =>
        this.accessCodeService.saveFirstMessagesLog({
          ...
          phone_number: ReminderThreeDto.wa_no,
          file_location: `${link}${ReminderThreeDto.file_name}`,
          debtor_name: ReminderThreeDto.due_no,
